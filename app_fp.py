...
from network_6 import define_stn_network, predicted_stocks

results_list = []
H_Values = range(525, 526, 25)
beta_min_factors = np.arange(1, 0.74, -0.25)
tau_max_factor = 1
    ...
    for beta_min_factor in beta_min_factors:        
        ...
        if (results.solver.termination_condition != TerminationCondition.infeasible):
            ...
            for k in model.S_Materials:                    
                model.P_Product_Production[k] = sum(model.V_B[i,j,n].value for i in model.S_I_Producing_K[k] for j in model.S_J_Executing_I[i] for n in model.S_Time)
        ...
        """ results_list.append({
            ...
            "Gap New Formulation %": 100*(lp_objective_value - objective_value)/lp_objective_value,
