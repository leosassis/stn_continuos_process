...
from src.models.constraints import (load_constraints_basic_model, 
                                    load_constraints_basic_model_for_operations_x_y_unit, 
                                    load_constraints_basic_model_for_operations_x_y_task)
...
def load_basic_model_constraints_objective(model: ConcreteModel, stn_data: dict, planning_horizon: int, model_type: str) -> None:
    ...
    Args:
        ...
        - stn_data (dict): a dictionary with the state-task network instance data.
        - planning_horizon (int): planning horizon.
        - model_type (str): a string indicating which base model to load (e.g., 'base_model', 'bound_production_operations_x_unit', 'bound_startups_ys_unit')
    ...
    elif model_type == 'bound_production_operations_x_unit':
        load_constraints_basic_model_for_operations_x_y_unit(model)
        create_objective_function(model, stn_data, 'bound_production_operations')
    elif model_type == 'bound_startups_ys_unit':
        load_constraints_basic_model_for_operations_x_y_unit(model)
