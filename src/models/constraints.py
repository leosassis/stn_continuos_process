...
def track_idle_unit_eq13(model, j, n):
    ...
    if (
        j in model.S_J_Units_With_Shutdown_Tasks
    ):
        return model.V_X_Hat_Idle[j,n] == (
                                        (model.V_X_Hat_Idle[j,n-1] if n >= 1 else 0)
                                      + (model.P_Unit_Initialization[j] if n == 0 else 0)  
                                      + sum(
                                          model.V_X[ii,j,n-model.P_Tau[ii,j]] 
                                          for i in model.S_I_Production_Tasks_With_Indirect_Transition
                                          for ii in model.S_I_Shutdown_Tasks
                                          if (j,i,ii) in model.P_Task_Transitions_Unit
                                          if model.P_Task_Transitions_Unit[j,i,ii] == -1
                                          if n-model.P_Tau[ii,j] >= 0)
...
def unit_availability_eq21_new(model, j, n):
...
def load_constraints_basic_model(model: ConcreteModel) -> None:
    ...
    #model.C_Unit_Availability_Eq21_New = Constraint(model.S_Units, model.S_Time, rule = unit_availability_eq21_new)
