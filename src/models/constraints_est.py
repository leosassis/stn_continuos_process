...
def est_constraint_upper_bound_number_of_runs(model: ConcreteModel, i: Any, j: Any) -> Constraint:
    if (i in model.S_I_Production_Tasks) and (j in model.S_J_Executing_I[i]) and ((i,j) in model.P_Task_Unit_Network):
        return sum(model.V_Y_Start[i,j,n] for n in model.S_Time if n >= model.P_EST[i,j]) <= floor((max(model.S_Time) + 1 - model.P_EST[i,j])/(model.P_Tau_Min[i,j] + model.P_Tau_End_Task[i]))
    else:
        return Constraint.Skip    


def est_constraint_upper_bound_number_of_runs_unit(model: ConcreteModel, j: Any) -> Constraint:
    return sum(model.V_Y_Start[i,j,n] for i in model.S_I_Production_Tasks if (i,j) in model.P_Task_Unit_Network for n in model.S_Time if n >= model.P_EST_Unit[j]) <= floor((max(model.S_Time) + 1 - model.P_EST_Unit[j])/(min(model.P_Tau_Min[i,j] for i in model.S_I_Production_Tasks if (i,j) in model.P_Task_Unit_Network) + model.P_Tau_End_Unit[j]))
