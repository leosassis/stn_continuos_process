...
def material_mass_balance_eq3(model, k, n):
    if k not in model.S_Raw_Materials:    
        return model.V_S[k,n] == ((model.V_S[k,n-1] if n >= 1 else 0) + (model.P_Init_Inventory_Material[k] if n == 0 else 0)
                               ...
                               #                 #if t >= 0 and t <= n - model.P_Tau[i,j]
                               #                 if n - model.P_Tau[i,j] >= 0
                               #             )
                                + sum(model.P_Rho_Plus[i,k]*model.V_B[i,j,n] for i in model.S_I_Producing_K[k] for j in model.S_J_Executing_I[i] 
                                                if i in model.S_I_Production_Tasks
                                                #for (i,j) in model.P_Task_Unit_Network
                                                #if i in model.S_I_Producing_K[k] 
                                                #for t in model.S_Time 
                                                #if t >= n and t <= n - model.P_Tau[i,j]
                                                #if n - model.P_Tau[i,j] >= 0
                                    )
                                + sum(model.P_Rho_Plus[i,k]*model.V_B[i,j,t] for t in model.S_Time for i in model.S_I_Producing_K[k] for j in model.S_J_Executing_I[i] 
    ...
    if i in model.S_I_Production_Tasks and j in model.S_J_Executing_I[i]:
        return model.V_X_Hat[i,j,n] == (
                                        ...
                                        + sum(model.V_X[ii,j,n-model.P_Tau[ii,j]] 
                                            ...
                                            #if ii in model.S_I_Startup_Tasks
                                        ...
                                        - sum(
                                            ...
                                            if (j,i,ii) in model.P_Task_Transitions_Unit 
                                            if model.P_Task_Transitions_Unit[j,i,ii] == -1
                                            #if ii in model.S_I_Indirect_Transition_Tasks    
                                            #if ii not in model.S_I_Startup_Tasks
                                        )
        )                                         
    else:
        return Constraint.Skip
    ...
    if j in (model.S_J_Units_With_Direct_Transition_Tasks or model.S_J_Units_With_Shutdown_Tasks) and (i,j) in model.P_Task_Unit_Network and i in model.S_I_Production_Tasks:
        return model.V_X_Hat[i,j,n] == ((model.V_X_Hat[i,j,n-1] if n >= 1 else 0)
                                         ...
                                         + sum(
                                            ...
                                            if (j,i,ii) in model.P_Task_Transitions_Unit 
                                            if model.P_Task_Transitions_Unit[j,i,ii] == 1
                                            #if ii in model.S_I_Direct_Transition_Tasks
...
#def min_lenght_run_eq18_reformulation_startup(model, i, j, n):
#    if (i,j) in model.P_Task_Unit_Network and i in (model.S_I_Production_Tasks & ):
#        return model.V_X[i,j,n] >= sum(
#                                        model.V_Y_Start[i,j,nprime] 
#                                        for nprime in model.S_Time 
...
def max_lenght_run_eq19(model, i, j, n):
    if (i,j) in model.P_Task_Unit_Network and i in model.S_I_Production_Tasks:
        return sum(
                    ...
                    for nprime in model.S_Time 
                    if (nprime >= n - model.P_Tau_Max[i,j] and nprime <= n)
                  ) <= model.P_Tau_Max[i,j] 
    else:
        return Constraint.Skip
...
def unit_availability_eq21(model, j, n):
    return (
      ...
      + (model.V_X_Hat_Idle[j,n] if j in model.S_J_Units_With_Shutdown_Tasks else 0)
...
def create_constraints(model, STN, H):
    ...
    #model.C_Track_Startup_Shutdown_Eq12 = Constraint(model.S_Tasks, model.S_Units, model.S_Time, rule = track_transitions_unit_eq12)
...
def add_tau_max_reformulation_YS(model):
...
def add_tau_max_reformulation_YE(model):
   model.C_Max_Lenght_Run_Eq19.clear()
